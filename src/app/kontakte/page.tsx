...
export default function KontaktePage() {
  return (
    <>
      ...
      <main className="mx-auto w-full max-w-[860px] flex-grow px-6 py-8">
      ...
      {accepted.length > 0 && (
        <section className="flex flex-col gap-4">
          ...
          {accepted.map((c) => (
            <Link
              key={c.connection_id}
              href={`/kontakte/${c.connection_id}`}
              className="press flex items-center gap-4 rounded-card border border-line bg-raised p-5 text-ink no-underline"
            >
              <Avatar name={c.other_name} size={64} />
